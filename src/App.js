...
class App extends Component {
  ...
  fetchData = () => {
    this.state.popularName
      ? fetch(
          `https://api.github.com/repos/${this.state.popularName}/issues?page=${this.state.page}`
        )
          .then((res) => res.json())
          .then((data) => {
            if (data.length) {
              this.props.dispatch({
                type: "ADD_DATA",
                payload: data.map((v) => ({ ...v, isClicked: false })),
              });
            } else {
      ...
      : fetch(
          `https://api.github.com/repos/${
            this.state.value ? this.state.value.toLowerCase() : "facebook/react"
  ...
  incrementPage = () => {
    this.setState((prevState) => ({ page: ++prevState.page, loading: true }));
    this.fetchData();
  };
  ...
  render() {
    let filtered = this.props.defaultArray.filter((val) => {
      return val.labels.some(
        (obj) => obj.name.split(":")[0] === this.state.filterLabel
      );
    });
        ...
        {this.state.loading ? (
          ...
          <>
            ...
            <Card
              ...
              data={
                filtered.length && !isFilter
                  ? filtered
                  : this.props.defaultArray
              }
            />
          </>
        )}
